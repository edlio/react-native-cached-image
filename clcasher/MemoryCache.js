...
MemoryCache.set = async (key, value, expires = DEFAULT_EXPIRES) => {
    ...
    try {
        // There is apparently a bug in which this _first_ time this function is called per VM, it fails to set a value.
        if (!MemoryCache.hasSetOnce) {
          MemoryCache.hasSetOnce = true
          await MemoryCache.set(key,value,expires);
        }
